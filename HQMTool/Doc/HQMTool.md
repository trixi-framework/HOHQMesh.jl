...
The mesh file format is either `ISM` or `ISM-V2`. The plot file (Which can be viewed with something like VisIt or Paraview) format is either `skeleton` or `sem`. The former is just a low order finite element represntation of the mesh. The latter (which is a much bigger file) includes the interior degrees of freedom. **At this time, if you want to plot the grid in HQMTool, then you must use `ISM-V2` as the mesh file format.**
